...
public class CsvFileWriter {
	...
	public static void writeCsvFile(String fileName, List<Record> records) {
		...
		/*
		 * //Create new Records objects Record Record1 = new Record("Rec001",
		 * 101, "ChristmasDist", 12.0, null, 111.50, "Cash", true,
		 * "ValidChrist", "Chicago"); Record Record2 = new Record("Rec001", 101,
		 * "ChristmasDist", 12.0, null, 111.50, "Cash", true, "ValidChrist",
		 * "Chicago"); Record Record3 = new Record("Rec001", 101,
		 * "ChristmasDist", 12.0, null, 111.50, "Cash", true, "ValidChrist",
		 * "Chicago"); Record Record4 = new Record("Rec001", 101,
		 * "ChristmasDist", 12.0, null, 111.50, "Cash", true, "ValidChrist",
		 * "Chicago"); Record Record5 = new Record("Rec001", 101,
		 * "ChristmasDist", 12.0, null, 111.50, "Cash", true, "ValidChrist",
		 * "Chicago"); Record Record6 = new Record("Rec001", 101,
		 * "ChristmasDist", 12.0, null, 111.50, "Cash", true, "ValidChrist",
		 * "Chicago");
		 * 
		 * //Create a new list of Record objects List<Record> Records = new
		 * ArrayList<Record>(); Records.add(Record1); Records.add(Record2);
		 * Records.add(Record3); Records.add(Record4); Records.add(Record5);
		 * Records.add(Record6);
		 */
		...
		try {
			...
			for (Record Record : records) {
				if (Record.getMessages().isEmpty()) {
					...
					corrFileWriter.append(COMMA_DELIMITER);
					corrFileWriter.append(String.valueOf(Record
							.getContract_date()));
					corrFileWriter.append(COMMA_DELIMITER);
